...
function App() {
  ...
  // --- Auto-Sync Logic (Ensures Data Persistence) ---
  const fetchData = useCallback(async () => {
    ...
    } catch (e) { console.warn("Sync error - check connection"); }
  ...
  const processReward = async (id, amt) => {
    // Reward logic for Bot/Social (Only once)
  ...
  const styles = {
    ...
    input: { width: '100%', padding: '10px', marginBottom: '10px', borderRadius: '8px', border: '1px solid #000', boxSizing: 'border-box', color: '#000' },
  ...
  return (
    <div style={styles.main}>
      <div style={styles.header}>
        <small>{isSyncing ? "SYNCING..." : "CURRENT BALANCE"}</small>
            ...
            {activeTab === 'admin' && (
              <div>
                ...
                <button style={styles.btn} onClick={async () => {
                   ...
                   alert("Task Added!"); setAdminTaskName(''); fetchData();
                }}>Save Task</button>
              </div>
            )}
          </div>
      ...
      {activeNav === 'invite' && (
        <div style={styles.card}>
          ...
          <p>Get <b>{APP_CONFIG.REFER_REWARD} TON</b> for every friend who joins!</p>
      ...
      {activeNav === 'withdraw' && (
        <>
          <div style={styles.card}>
            <h3>VIP Upgrade ({APP_CONFIG.VIP_PRICE} TON)</h3>
          ...
          <div style={styles.card}>
            ...
            <input style={styles.input} placeholder="Recipient TON Address" onChange={e => setWithdrawAddress(e.target.value)} />
            <button style={{...styles.btn, background:'#3b82f6'}} onClick={() => handleAction(async () => {
              const amount = Number(withdrawAmount);
              if(amount < 0.1) return alert("Minimum is 0.1 TON.");
              ...
              alert("Request sent! Will confirm in 5 minutes."); 
              fetchData();
              setWithdrawAmount('');
            })}>WITHDRAW NOW</button>
          </div>
