...
function App() {
  ...
  const [adsterraId, setAdsterraId] = useState("29164006"); 
  const [newAdsgramInput, setNewAdsgramInput] = useState('');
  const [newAdsterraInput, setNewAdsterraInput] = useState('');
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (adsData) {
        ...
        setAdsterraId(adsData.adsterraId || "29164006");
      }
  ...
  // --- Hybrid Reward Logic: Both Adsterra and Adsgram ---
  const processReward = (id, rewardAmount) => {
    ...
    // STEP 1: Open Adsterra (Browser Link)
    const adLink = adsterraId.startsWith('http') ? adsterraId : `https://www.profitablecpmrate.com/${adsterraId}`;
    tg?.openLink ? tg.openLink(adLink) : window.open(adLink, '_blank');

    // STEP 2: Show Adsgram (Video)
    if (window.Adsgram) {
      ...
      AdController.show().then((result) => {
        if (result.done) {
          ...
          alert(`Success! Adsterra + Video Complete: +${finalReward} TON`);
          fetchData();
        }
      }).catch(e => alert("Adsgram loading error, please try again."));
    }
  };
  ...
  return (
    <div style={styles.main}>
      ...
      {activeNav === 'earn' && (
        <>
          <div style={{...styles.card, background: '#000', color: '#fff', textAlign: 'center'}}>
             <p style={{margin: '0 0 10px 0', fontWeight: 'bold'}}>Get Reward (Direct Link + Video)</p>
          ...
          <div style={styles.card}>
            ...
            {activeTab === 'admin' && (
              <div>
                <h4>Admin Control</h4>
                {/* --- Hybrid Ads Manager --- */}
                <h5 style={{color: '#3b82f6'}}>📺 ADS MANAGER (Hybrid)</h5>
                <input style={styles.input} placeholder="Adsgram Block ID" value={newAdsgramInput} onChange={e => setNewAdsgramInput(e.target.value)} />
                <input style={styles.input} placeholder="Adsterra Smartlink/ID" value={newAdsterraInput} onChange={e => setNewAdsterraInput(e.target.value)} />
                <button style={{...styles.btn, background: '#3b82f6', marginBottom: '15px'}} onClick={async () => {
                   if(!newAdsgramInput || !newAdsterraInput) return alert("Fill all IDs");
                   await fetch(`${APP_CONFIG.FIREBASE_URL}/settings/ads_config.json`, { 
                     method: 'PATCH', 
                     body: JSON.stringify({ adsgramId: newAdsgramInput, adsterraId: newAdsterraInput }) 
                   });
                   alert("Ads Updated!"); fetchData();
                }}>UPDATE HYBRID ADS</button>
                <hr style={{margin: '15px 0'}}/>
                
                {/* User Search & Other Admin tools continue as before... */}
              </div>
            )}
          </div>
        </>
      ...
      {/* Other Navigation Tabs (Invite, Withdraw, Profile) remain the same... */}
