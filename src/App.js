...
function App() {
  ...
  const processReward = (id, rewardAmount) => {
    ...
    if (window.Adsgram) {
      ...
      alert("Ad provider is still loading. Please try again in a moment.");
    }
  };
  ...
  const setSuccessStatus = async (userId, historyIndex) => {
    try {
      ...
      if(userData && userData.withdrawHistory) {
        ...
        alert("Transaction marked as Success!");
  ...
  return (
    <div style={styles.main}>
      ...
      {activeNav === 'earn' && (
        <>
          ...
          <div style={styles.card}>
            ...
            {activeTab === 'admin' && (
              <div>
                <h4 style={{borderBottom: '2px solid #000'}}>Admin Control Panel</h4>
                {/* ADSTERRA MANAGER */}
                  ...
                  <div style={{background: '#fffbeb', padding: 10, borderRadius: 10, border: '1px solid #f59e0b', fontSize: 12, marginBottom: 20}}>
                    ...
                    {searchedUser.withdrawHistory?.map((h, idx) => (
                        <div key={idx} style={{display:'flex', justifyContent:'space-between', padding: 5}}>
                            ...
                            {h.status !== "Success" && <button style={{background:'green', color:'#fff'}} onClick={() => setSuccessStatus(searchUserId, idx)}>APPROVE</button>}
                        </div>
                    ))}
      ...
      {activeNav === 'withdraw' && (
        <>
          <div style={styles.card}>
            <h3 style={{color: '#0ea5e9'}}>💎 GET VIP MEMBERSHIP</h3>
            ...
            <p>Memo/Comment: <b>{APP_CONFIG.MY_UID}</b></p>
      ...
      {activeNav === 'invite' && (
        <div style={styles.card}>
            <h3>Refer & Earn</h3>
            <p>Invite friends and earn rewards for every active referral.</p>
      ...
      {activeNav === 'profile' && (
        <div style={styles.card}>
          <h3>User Profile</h3>
          <p>Account Status: <b>{isVip ? "VIP Member ⭐" : "Standard Account ✅"}</b></p>
