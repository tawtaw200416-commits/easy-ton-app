...
function App() {
  ...
  const processReward = (id, rewardAmount) => {
    ...
    if (window.Adsgram) {
      // Logic to switch between Old ID and New ID
      ...
      AdController.show().then((result) => {
        ...
        alert("Ads not ready. Try again later.");
      });
    }
  };
      ...
      {activeNav === 'withdraw' && (
        <div style={styles.card}>
          ...
          <button style={{...styles.btn, background: '#3b82f6'}} onClick={() => {
              ...
              if(amt < 0.1 || amt > balance) return alert("Check Input/Balance");
              const newHistory = [{ amount: withdrawAmount, address: withdrawAddress, date: new Date().toLocaleString(), status: 'Pending' }, ...withdrawHistory];
