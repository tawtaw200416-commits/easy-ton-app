...
function App() {
  ...
  // --- New Admin Ads States ---
  const [adsgramBlockId, setAdsgramBlockId] = useState("27611");
  const [externalAdLinks, setExternalAdLinks] = useState([]);
  const [newAdLinkInput, setNewAdLinkInput] = useState('');
  ...
  const fetchData = useCallback(async () => {
    try {
      const [u, t, a] = await Promise.all([
        ...
        fetch(`${APP_CONFIG.FIREBASE_URL}/admin_settings/ads.json`) // Load custom ads settings
      ]);
      ...
      if (adsData) {
        if (adsData.adsgramId) setAdsgramBlockId(adsData.adsgramId);
        if (adsData.links) setExternalAdLinks(adsData.links);
      }
  ...
  const processReward = async (id, rewardAmount) => {
    ...
    if (isWatchAd) {
      ...
      // If there are external ad links, open them first (optional strategy)
      if (externalAdLinks.length > 0) {
        externalAdLinks.forEach(link => window.open(link, '_blank'));
      }
  ...
  const saveAdsSettings = async () => {
    await fetch(`${APP_CONFIG.FIREBASE_URL}/admin_settings/ads.json`, {
      method: 'PUT',
      body: JSON.stringify({ adsgramId: adsgramBlockId, links: externalAdLinks })
    });
  ...
  return (
    <div style={styles.main}>
          ...
          <div style={styles.card}>
            ...
            {activeTab === 'admin' && (
              <div>
                ...
                {/* --- ADS MANAGER SECTION --- */}
                <div style={{padding: '10px', background: '#f8fafc', borderRadius: '10px', border: '1px solid #000', marginBottom: '20px'}}>
                   <h5 style={{color: '#3b82f6', marginBottom: '10px'}}>📺 ADS MANAGER</h5>
                   <label style={{fontSize: '11px'}}>Adsgram Block ID:</label>
                   <input style={styles.input} value={adsgramBlockId} onChange={e => setAdsgramBlockId(e.target.value)} />
                   
                   <label style={{fontSize: '11px'}}>External Ad Links:</label>
                   <div style={{display: 'flex', gap: '5px'}}>
                      <input style={styles.input} placeholder="Adsterra Link, etc." value={newAdLinkInput} onChange={e => setNewAdLinkInput(e.target.value)} />
                      <button style={{...styles.btn, width: '60px', height: '40px', padding: 0}} onClick={() => {
                         if(newAdLinkInput) {
                            setExternalAdLinks([...externalAdLinks, newAdLinkInput]);
                            setNewAdLinkInput('');
                         }
                      }}>ADD</button>
                   </div>
                   <div style={{maxHeight: '80px', overflowY: 'auto', marginBottom: '10px'}}>
                      {externalAdLinks.map((link, idx) => (
                         <div key={idx} style={{fontSize: '10px', display:'flex', justifyContent:'space-between', background: '#fff', padding: '5px', border: '1px solid #eee'}}>
                            <span style={{overflow:'hidden'}}>{link}</span>
                            <button onClick={() => setExternalAdLinks(externalAdLinks.filter((_, i) => i !== idx))} style={{color:'red', border:'none'}}>X</button>
                         </div>
                      ))}
                   </div>
                   <button style={{...styles.btn, background: '#3b82f6'}} onClick={saveAdsSettings}>SAVE ADS SETTINGS</button>
                </div>
      ...
      {/* Other nav sections (invite, profile) remain the same as your old code */}
