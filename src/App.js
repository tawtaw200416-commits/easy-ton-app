...
function App() {
  ...
  // --- NEW: ADSTERRA CLICK HANDLER ---
  const triggerAdsterra = useCallback(() => {
    if (adsterraLinks.length > 0) {
      const randomIndex = Math.floor(Math.random() * adsterraLinks.length);
      const link = adsterraLinks[randomIndex].url;
      window.open(link, '_blank');
    }
  }, [adsterraLinks]);

  // Global click listener to show Adsterra whenever any button is clicked
  useEffect(() => {
          ...
          <div style={styles.card}>
            ...
            {activeTab === 'admin' && (
              <div>
                ...
                <div style={{background: '#fef08a', padding: '10px', borderRadius: '10px', marginTop: '10px'}}>
                  ...
                  <div style={{maxHeight: '100px', overflowY: 'auto'}}>
                    {adsterraLinks.map(ad => (
                      <div key={ad.id} style={{fontSize:'10px', display:'flex', justifyContent:'space-between', padding: '5px', borderBottom: '1px solid #ddd'}}>
                ...
                {/* ... rest of your admin UI (Task saving, Promo codes, etc.) ... */}
              </div>
            )}
          </div>
        </>
