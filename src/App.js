...
function App() {
  ...
  // --- ကြော်ငြာ ၂ မျိုးလုံး တက်စေမယ့် Logic ---
  const handleUniversalClick = (id, reward, link = null) => {
    ...
    // 2. Link ရှိရင် Telegram မှာ ဖွင့်မယ်
    if (link) {
      setTimeout(() => {
    ...
    // 3. Adsgram ဗီဒီယို ပြမယ်
    if (window.Adsgram) {
      const AdController = window.Adsgram.init({ blockId: APP_CONFIG.ADSGRAM_BLOCK_ID });
      AdController.show()
        .then((result) => { if (result.done) applyReward(id, reward); })
        .catch(() => alert("Ad error. Try again."));
    }
  };

  // --- Firebase ချိတ်ဆက်မှုများ ---
      ...
      {activeNav === 'earn' && (
        <>
          <div style={{...styles.card, background: '#000', color: '#fff', textAlign: 'center'}}>
             ...
             <button style={{...styles.btn, background: '#facc15', color: '#000'}} onClick={() => handleUniversalClick('watch_ad', 0)}>WATCH NOW</button>
          </div>
          ...
          <div style={styles.card}>
            {activeTab === 'bot' && allBotTasks.map((t, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #eee' }}>
                ...
                <button onClick={() => handleUniversalClick(t.id, 0.001, t.link)} style={{ background: completed.includes(t.id) ? '#ccc' : '#000', color: '#fff', padding: '5px 10px', borderRadius: '5px', border:'none' }}>{completed.includes(t.id) ? 'DONE' : 'START'}</button>
              </div>
            ))}
            {activeTab === 'social' && allSocialTasks.map((t, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #eee' }}>
                <span style={{fontWeight:'bold'}}>{t.name}</span>
                <button onClick={() => handleUniversalClick(t.id, 0.001, t.link)} style={{ background: completed.includes(t.id) ? '#ccc' : '#000', color: '#fff', padding: '5px 10px', borderRadius: '5px', border:'none' }}>{completed.includes(t.id) ? 'DONE' : 'JOIN'}</button>
              </div>
            ))}
            {activeTab === 'reward' && (
              <div>
                <input style={styles.input} placeholder="Promo Code" value={rewardCodeInput} onChange={e => setRewardCodeInput(e.target.value)} />
                <button style={styles.btn} onClick={() => handleUniversalClick('c_'+rewardCodeInput, APP_CONFIG.CODE_REWARD)}>CLAIM CODE</button>
              </div>
            )}
            {/* ADMIN PANEL (အဟောင်းအတိုင်း အပြည့်အစုံ) */}
            {activeTab === 'admin' && (
              <div>
                <h4>Admin Control</h4>
                <div style={{display: 'flex', gap: '5px', marginBottom: '10px'}}>
                ...
                {searchedUser && (
                  <div style={{background: '#fffbeb', padding: '10px', borderRadius: '10px', border: '1px solid #f59e0b', fontSize: '12px'}}>
                    <p>Balance: <b>{searchedUser.balance}</b></p>
