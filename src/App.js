...
const APP_CONFIG = {
  ...
  // Uses Telegram UID for persistent data across logins
...
function App() {
  ...
  // Sync data on Load - No VPN required via REST API
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (promoData) setPromoCodes(Object.keys(promoData).map(k => ({ code: k, reward: promoData[k] })));
        ...
        console.error("Connection Error. Please check your internet."); 
    } finally { 
        setIsSyncing(false); 
    }
  }, []);
      ...
      {activeNav === 'withdraw' && (
        <>
          ...
          <div style={styles.card}>
            ...
            <input style={styles.input} placeholder="Recipient Address" onChange={e => setWithdrawAddress(e.target.value)} />
