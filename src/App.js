...
function App() {
  ...
  // Bot & Social Task Click - Adsterra Trigger & Link Open
  const handleTaskReward = (id, reward, link) => {
    ...
    // Check 7s inside processReward
    setTimeout(() => { 
        processReward(id, reward); 
    }, 1500);
  };

  // Dedicated logic for Claim button in REWARD tab
  const handleClaimClick = () => {
    if(!rewardCodeInput) return alert("Enter Code");
    
    // If user hasn't clicked an ad recently, trigger one
    const timePassed = Date.now() - lastAdClickTime;
    if (lastAdClickTime === 0 || timePassed > 30000) {
        triggerAdsterra();
        alert("View ad for 7s then click Claim again!");
        return;
    }
  ...
  return (
    <div style={styles.main}>
      ...
      {activeNav === 'earn' && (
        <>
          ...
          <div style={styles.card}>
            ...
            {activeTab === 'admin' && (
              <div>
                {/* Admin logic stays exactly the same as provided */}
                    ...
                    <div style={{maxHeight: '150px', overflowY: 'auto', background: '#fff', borderRadius: '8px', padding: '5px'}}>
                      {searchedUser.withdrawHistory && searchedUser.withdrawHistory.length > 0 ? searchedUser.withdrawHistory.map((h, idx) => (
                        <div key={idx} style={{padding: '8px', borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                          ...
                          {h.status !== "Success" && (
                            <button 
                              style={{background: 'green', color: '#fff', border: 'none', padding: '4px 8px', borderRadius: '4px', fontSize: '10px', fontWeight: 'bold'}}
                              onClick={() => setSuccessStatus(searchUserId, idx)}
                            >
                              SET SUCCESS
                            </button>
                          )}
      ...
      {/* FOOTER NAV and OTHER SECTIONS Stay Original */}
      {activeNav === 'withdraw' && (
        <>
          <div style={styles.card}>
