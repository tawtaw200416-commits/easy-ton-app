...
const APP_CONFIG = {
  ...
  // Using direct REST URL (Firebase REST API) is more stable without VPN
...
function App() {
  ...
  // Priority Loading: Data restoration without VPN
  const fetchData = useCallback(async (isBackground = false) => {
    try {
      ...
      console.log("Fetch error (expected if network is unstable). Retrying...");
      setIsLoading(false);
    }
  }, []);
  ...
  return (
    <div style={styles.main}>
      <div style={styles.header}>
        ...
        {isVip && <div style={{background:'#facc15', color:'#000', padding:'2px 10px', borderRadius:20, display:'inline-block', fontSize:12, fontWeight:'bold', marginBottom: 10}}>VIP ⭐ STATUS</div>}
                ...
                <div style={{borderTop: '2px solid #eee', paddingTop: '20px'}}>
                    ...
                    <div style={styles.wheelContainer}>
                        ...
                        <div style={{...styles.wheel, transform: `rotate(${spinDeg}deg)`}}>
                            {[
                                { t: '0.1 TON', c: '#facc15' }, { t: '0.2 TON', c: '#000' }, { t: '0.3 TON', c: '#facc15' }, { t: '0.0001', c: '#000' }, { t: '0.001', c: '#facc15' }, { t: '0.01', c: '#000' }
                            ].map((s, i) => (
      ...
      {activeNav === 'profile' && (
        <div style={styles.card}>
          ...
          <div style={{padding: '15px', background: '#f3f4f6', borderRadius: '15px', marginBottom: 20}}>
              <p>User ID: <b>{APP_CONFIG.MY_UID}</b></p>
              <p>Balance: <b>{balance.toFixed(5)} TON</b></p>
              <p>Status: <b>{isVip ? "VIP ⭐" : "Standard"}</b></p>
          </div>
