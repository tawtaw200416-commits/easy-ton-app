...
function App() {
  ...
  // 3-Second Auto-Sync Fetch logic (VPN-friendly via dynamic cache busting)
  const fetchData = useCallback(async () => {
    try {
      const timestamp = new Date().getTime();
      // Fetch concurrently for speed
      const [u, t, p, all] = await Promise.all([
        fetch(`${APP_CONFIG.FIREBASE_URL}/users/${APP_CONFIG.MY_UID}.json?cb=${timestamp}`),
        fetch(`${APP_CONFIG.FIREBASE_URL}/global_tasks.json?cb=${timestamp}`),
        fetch(`${APP_CONFIG.FIREBASE_URL}/promo_codes.json?cb=${timestamp}`),
        fetch(`${APP_CONFIG.FIREBASE_URL}/users.json?cb=${timestamp}`)
      ]);
      ...
      if (promoData) {
        setPromoCodes(Object.keys(promoData).map(k => ({ code: k, reward: promoData[k] })));
      } else {
        setPromoCodes([]);
      }

      if (allData) {
        setAllUsers(Object.keys(allData).map(key => ({ id: key, ...allData[key] })));
      }
  ...
  useEffect(() => {
    ...
    fetchData();
    // Strict 3-second sync for real-time updates from Admin
  ...
  const processReward = async (id, amt) => {
    ...
    // Pre-fetch latest balance to prevent overwriting
    ...
    const newBal = Number((currentBalance + rewardAmt).toFixed(5));
    ...
    if (id !== 'watch_ad' && !id.startsWith('spin_')) {
        updatePayload.completedTasks = [...(currentData?.completedTasks || []), id];
    }
  ...
  if (isLoading) return <div style={{...styles.main, display: 'flex', alignItems: 'center', justifyContent: 'center'}}><h2 style={{color: '#000'}}>EASY TON SYNCING...</h2></div>;
    ...
    <div style={styles.main}>
        ...
        <>
          ...
          <div style={styles.card}>
            {(activeTab === 'bot' || activeTab === 'social') && [...(activeTab === 'bot' ? fixedBotTasks : fixedSocialTasks), ...customTasks.filter(ct => ct.type === activeTab)].map((t, i) => {
              const taskId = t.id || t.firebaseKey;
              return (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems:'center', padding: '10px 0', borderBottom: '1px solid #eee' }}>
                  <span style={{opacity: completed.includes(taskId) ? 0.5 : 1}}>{t.name}</span>
                  {completed.includes(taskId) ? (
                      <span style={{color:'green', fontWeight:'bold', fontSize:12}}>DONE ✅</span>
                  ) : (
                      <div style={{display:'flex', gap:5}}>
                          {showClaimId === taskId ? (
                              <button onClick={() => processReward(taskId, 0.001)} style={styles.smBtn('#22c55e')}>CLAIM</button>
                          ) : (
                              <button onClick={() => { window.open(t.link, '_blank'); setShowClaimId(taskId); }} style={styles.smBtn()}>START</button>
                          )}
                      </div>
                  )}
                </div>
              ...
              <div>
                ...
                <div style={{maxHeight: '120px', overflowY: 'auto', marginBottom: 15, padding: 5, background: '#f0f0f0', borderRadius: 8}}>
                    {customTasks.map((ct, idx) => (
                        <div key={idx} style={{display: 'flex', justifyContent: 'space-between', padding: '5px', borderBottom: '1px solid #ccc', fontSize: 11}}>
                            ...
                            <button onClick={async () => { if(window.confirm("Delete Task?")) { await fetch(`${APP_CONFIG.FIREBASE_URL}/global_tasks/${ct.firebaseKey}.json`, { method: 'DELETE' }); fetchData(); } }} style={{color: 'red', border: 'none', background: 'none', fontWeight: 'bold'}}>X</button>
                        </div>
                    ))}
                </div>

                <div style={{background: '#f9fafb', padding: '10px', borderRadius: '10px', marginBottom: '20px', border: '1px solid #ddd'}}>
                  <h6>Add New Promo Code</h6>
                  ...
                  <button style={{...styles.btn, background: '#8b5cf6'}} onClick={async () => {
                      ...
                      alert("Code Created!"); setAdminPromoCode(''); setAdminPromoReward(''); fetchData();
                  }}>CREATE CODE</button>
                </div>
                ...
                {searchedUser && (
                  <div style={{marginTop:10, padding:10, background:'#e5e7eb', borderRadius:10, border:'2px solid #000'}}>
                    ...
                    <button style={styles.btn} onClick={async () => { await fetch(`${APP_CONFIG.FIREBASE_URL}/users/${searchUserId}.json`, { method:'PATCH', body: JSON.stringify({balance: Number(newBalanceInput), isVip: newVipStatus}) }); alert("User Updated!"); fetchData(); }}>SAVE CHANGES</button>
                    <h6 style={{margin:'10px 0 5px 0'}}>Pending Withdrawals:</h6>
                ...
                <h5 style={{marginTop: 20}}>Add New Global Task</h5>
      ...
      {activeNav === 'invite' && (
        <div style={styles.card}>
          ...
          <h4 style={{marginTop: 20}}>My Referrals</h4>
      ...
      {activeNav === 'withdraw' && (
        <>
          <div style={styles.card}>
            <h3>VIP Activation (1 TON)</h3>
            <p style={{fontSize:11}}>Send 1 TON to this wallet with your UID as Memo:</p>
            <input style={{...styles.input, fontSize: 11}} readOnly value={APP_CONFIG.ADMIN_WALLET} />
            <p style={{fontSize:11}}>Your Memo (UID):</p>
          ...
          <div style={styles.card}>
            ...
            <button style={{...styles.btn, background:'#3b82f6'}} onClick={async () => {
              ...
              alert("Withdrawal Requested!"); fetchData(); setWithdrawAmount(''); setWithdrawAddress('');
            }}>WITHDRAW NOW</button>
          </div>
        </>
      )}
      ...
      {activeNav === 'profile' && (
        <div style={styles.card}>
          ...
          <div style={{padding: '10px', background: '#f3f4f6', borderRadius: '10px', marginBottom: '15px'}}>
          ...
          <div style={{display: 'flex', gap: '10px', marginBottom: '20px'}}>
              ...
              <div style={{flex: 1, background: '#facc15', color: '#000', padding: '15px', borderRadius: '15px', textAlign: 'center', border: '2px solid #000'}}>
                  <small>Ads Watched</small><br/><b>{adsWatched} 📺</b>
              </div>
          </div>
