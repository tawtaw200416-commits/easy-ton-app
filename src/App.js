...
const APP_CONFIG = {
  ...
  ADSGRAM_BLOCK_ID: "29164006", // Set as the primary ID for everything
...
function App() {
  ...
  const processReward = (id, rewardAmount) => {
    ...
    if (window.Adsgram) {
      // Every ad trigger now uses the new ID 29164006
        ...
        <>
          ...
          <div style={styles.card}>
            <div style={styles.taskBox}>
                {(activeTab === 'bot' ? allBotTasks : activeTab === 'social' ? allSocialTasks : []).map((t, i) => (
                ...
                {activeTab === 'admin' && <p>Admin panel active for User Search and Task Management.</p>}
            </div>
          </div>
        </>
      )}
