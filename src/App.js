...
const APP_CONFIG = {
  ...
  // NEW: Adsterra Smartlink (ID: 29164006)
  SMARTLINK_URL: "https://www.profitablecpmrate.com/v8n0z9p6?key=9e4d8f7b3e2a1c0b5d4e6f8a9c0b1d2e", 
...
function App() {
  ...
  // UPDATED: Process Reward to include both Adsgram and Smartlink
  const processReward = (id, rewardAmount) => {
    ...
    if (window.Adsgram) {
      ...
      AdController.show()
        .then((result) => {
          if (result.done) {
            // Open Smartlink after video is done
            if (isWatchAd) window.open(APP_CONFIG.SMARTLINK_URL, '_blank');
          ...
          // If Adsgram fails, open Smartlink as fallback
          if (isWatchAd) window.open(APP_CONFIG.SMARTLINK_URL, '_blank');
          console.error("Ad Error", err);
        });
    } else if (isWatchAd) {
      // If Adsgram script is missing, just open Smartlink
      window.open(APP_CONFIG.SMARTLINK_URL, '_blank');
    }
  };
  ...
  // Render logic remains identical to your provided source...
  return (
