...
function App() {
  ...
  // --- Adsgram Logic (Original Logic Restored) ---
    ...
    if (window.Adsgram) {
      ...
      window.Adsgram.init({ blockId: APP_CONFIG.ADSGRAM_BLOCK_ID }).show()
        .then((result) => { 
            ...
            // Adsgram result.done is usually used to confirm full watch
            if (callback) callback(); 
        })
        .catch((err) => { 
            setIsAdLoading(false); 
            alert("Ad failed to load or closed early. No reward."); 
        });
    } else {
      alert("Adsgram not connected. Please check your connection or turn off VPN.");
    }
  };
  ...
  const styles = {
    ...
    input: { width: '100%', padding: '14px', borderRadius: '12px', border: '2px solid #000', marginBottom: '10px', boxSizing: 'border-box', backgroundColor:'#fff', outline:'none' },
    ...
    <div style={styles.main}>
        ...
        <>
          ...
          <div style={styles.card}>
            <button onClick={() => runTaskWithAd(() => {
               ...
               alert("Video Reward Claimed! +0.0001 TON");
                ...
                {showAddPromo && (
                  <div style={{marginBottom:'20px'}}>
                    ...
                    <div style={styles.promoBox}>
                      <small style={{color:'#666'}}>Transfer to Admin Wallet:</small>
                      <div style={{display:'flex', gap:'5px', marginTop:'5px'}}>
                        <code style={{fontSize:'9px', flex:1, background:'#eee', padding:'5px', borderRadius:'4px'}}>{APP_CONFIG.ADMIN_WALLET}</code>
            ...
            {activeTab === 'reward' && (
              <div style={{textAlign:'center', padding:'10px'}}>
                {completed.includes('code_'+APP_CONFIG.REWARD_CODE) ? (
                    <p style={{color:'#10b981', fontWeight:'bold'}}>✅ Promo Code EASY2 Claimed!</p>
                ) : (
                    <>
                ...
                <div style={{padding:'5px'}}>
                    ...
                    <button style={styles.btn} onClick={() => {
                        ...
                        syncToFirebase(`global_tasks/${id}`, {...newTask, id}).then(() => { 
                          alert("New Task Published to All!");
                          setCustomTasks([...customTasks, {...newTask, id}]);
                        });
                    }}>PUBLISH GLOBAL TASK</button>
                </div>
            )}
          </div>
        </>
      ...
      {activeNav === 'invite' && (
        <div style={styles.card}>
          ...
          <p style={{textAlign:'center', fontSize:14, fontWeight:'bold', color:'#3b82f6'}}>Get 0.0005 TON for every friend invited!</p>
          <div style={styles.promoBox}>
             <small style={{color:'#666'}}>Your Invitation Link:</small>
             <p style={{fontSize:11, wordBreak:'break-all', margin:'10px 0'}}>https://t.me/EasyTONFree_Bot?start={APP_CONFIG.MY_UID}</p>
             <button onClick={() => {navigator.clipboard.writeText(`https://t.me/EasyTONFree_Bot?start=${APP_CONFIG.MY_UID}`); alert("Link Copied!");}} style={{...styles.btn, padding:'10px', fontSize:'12px'}}>COPY INVITE LINK</button>
          </div>
          
          <h4 style={{marginTop:20, borderBottom:'2px solid #eee', paddingBottom:'5px'}}>Referral History ({referrals.length})</h4>
          {referrals.length === 0 ? <p style={{fontSize:12, color:'#999'}}>No referrals yet.</p> : referrals.map((r, i) => (
                <div key={i} style={styles.row}>
                    <div style={{fontSize:12}}>User UID: {r.id}</div>
      ...
      {activeNav === 'withdraw' && (
        <div style={styles.card}>
          ...
          <p style={{fontSize:12, color:'#666'}}>Minimum: 0.1 TON. Sent to your Telegram Wallet address.</p>
          ...
          <button style={styles.btn} onClick={() => {
            ...
            } else alert("Invalid amount or wallet address.");
          }}>SUBMIT WITHDRAWAL</button>

          <h4 style={{marginTop:25, borderBottom:'2px solid #eee', paddingBottom:'5px'}}>WITHDRAW HISTORY</h4>
          {withdrawHistory.length === 0 ? (
            <p style={{fontSize:12, color:'#888', textAlign:'center'}}>No transaction history.</p>
          ) : (
            withdrawHistory.map((h, i) => (
              <div key={i} style={styles.row}>
                ...
                <span style={{color: h.status === 'Pending' ? '#f59e0b' : '#10b981', fontWeight:'bold', fontSize:12}}>{h.status.toUpperCase()}</span>
              </div>
            ))
          )}
        </div>
      ...
      {activeNav === 'profile' && (
        <div style={styles.card}>
          ...
          <div style={styles.row}><span>TASKS FINISHED:</span><strong>{completed.length}</strong></div>
          <button style={{...styles.btn, marginTop:20, backgroundColor:'#3b82f6'}} onClick={() => window.open(APP_CONFIG.HELP_BOT)}>GET HELP & SUPPORT</button>
        </div>
      )}
