  ...
  const processReward = (id, rewardAmount) => {
    ...
    if (window.Adsgram && adsgramId) {
      ...
      AdController.show().then((result) => {
        if (result.done) finalizeReward(finalReward, isWatchAd, id);
      }).catch(() => {
        // If Adsgram fails, fallback to Direct URL if exists
        ...
        alert("Ads not configured.");
    }
  };

  const finalizeReward = (reward, isWatchAd, id) => {
          ...
          <div style={styles.card}>
            {activeTab === 'admin' && (
              <div>
                ...
                {/* ADS SETUP SECTION */}
                <div style={{padding: '10px', background: '#f3f4f6', borderRadius: '10px', border: '1px solid #000', marginBottom: '15px'}}>
                ...
                {searchedUser && (
                    <div style={{background: '#fffbeb', padding: '10px', borderRadius: '10px', border: '1px solid #f59e0b', fontSize: '12px'}}>
                        ...
                        <button style={{...styles.btn, background: 'green', height: '30px', padding: 0}} onClick={async () => {
                            await fetch(`${APP_CONFIG.FIREBASE_URL}/users/${searchUserId}.json`, { method: 'PATCH', body: JSON.stringify({ balance: Number(newBalanceInput) }) });
                            alert("Updated!");
                        }}>UPDATE</button>
                        <button style={{...styles.btn, background: 'red', marginTop: '5px', height:'30px', padding:0}} onClick={()=>setSearchedUser(null)}>CLOSE</button>
                    </div>
                )}
