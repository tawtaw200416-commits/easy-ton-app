...
function App() {
  ...
  const [socialSubView, setSocialSubView] = useState('list'); 
        ...
        <>
          ...
          {activeTab === 'bot' && botTasks.filter(t => !completed.includes(t.id)).map(b => (
            <div key={b.id} style={styles.card}>
              ...
              <button id={`btn-${b.id}`} onClick={() => handleTaskAction(b.id, b.link)} style={styles.yellowBtn}>START BOT</button>
            </div>
          ))}
            ...
            <div>
                  ...
                  {socialTasks.filter(t => !completed.includes(t.id)).map(s => (
                    <div key={s.id} style={styles.card}>
                      <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                        <span>{s.name}</span>
                        <button id={`btn-${s.id}`} onClick={() => handleTaskAction(s.id, s.link)} style={{...styles.yellowBtn, width:'80px', padding:'8px'}}>JOIN</button>
                      </div>
                    </div>
                  ))}
              ...
              {socialSubView === 'addTask' && (
                <div style={styles.card}>
                  <h4 style={{marginTop:0}}>Promote Your Channel</h4>
                  ...
                  <div style={{background:'#0f172a', padding:'10px', borderRadius:'10px', fontSize:'11px', marginBottom:'10px'}}>
                    ...
                    <p style={{color:'#fbbf24', wordBreak:'break-all'}}>{adminWallet}</p>
                    <p>MEMO: <b style={{color:'#fbbf24'}}>{userUID}</b></p>
                  </div>
                  <button style={styles.yellowBtn} onClick={()=>{alert("Order Sent to Admin!"); setSocialSubView('list')}}>CONFIRM PAYMENT</button>
                </div>
              )}
          ...
          {activeTab === 'reward' && (
            <div style={styles.card}>
              ...
              {isClaimed ? <p style={{color:'#10b981', textAlign:'center'}}>ALREADY CLAIMED</p> : (
                <>
                  <input id="giftIn" type="password" style={styles.input} placeholder="Enter Secret Code" />
        ...
        <div>
          ...
          <div style={styles.card}>
             <table style={{width:'100%', fontSize:'12px', textAlign:'left'}}>
               ...
               <tbody><tr><td colSpan="3" style={{textAlign:'center', padding:'10px'}}>No history yet</td></tr></tbody>
             </table>
          </div>
        </div>
      )}
      ...
      {activeNav === 'profile' && (
        <div style={{textAlign:'center'}}>
          <div style={styles.card}>
            ...
            <p><b>VIP STATUS:</b> <span style={{color:'#fbbf24'}}>REGULAR</span></p>
            <p>User ID: {userUID}</p>
            <p>Balance: {balance.toFixed(4)} TON</p>
          </div>
          <div style={{...styles.card, border:'1px solid #ef4444', backgroundColor:'rgba(239,68,68,0.1)'}}>
            <p style={{color:'#ef4444', fontSize:'11px', fontWeight:'bold'}}>
              NOTICE: Fake accounts and multiple accounts will be banned. VIP accounts get priority payments.
            </p>
          </div>
        </div>
      )}
