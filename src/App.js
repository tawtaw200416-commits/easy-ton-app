...
const APP_CONFIG = {
  ...
  VIP_WATCH_REWARD: 0.0008, // Updated
  CODE_REWARD: 0.0008,
  REFER_REWARD: 0.01,
...
function App() {
  ...
  const triggerAdsSequence = useCallback(() => {
    ...
    // Trigger Adsterra randomly as the second ad
    setTimeout(() => {
  ...
  const processReward = async (id, amt) => {
    ...
    // For "Watch Ad" button specific reward logic
  ...
  const styles = {
    ...
    adminBtn: { padding: '8px', background: '#000', color: '#fff', borderRadius: '5px', fontSize: '12px', marginBottom: '5px', cursor: 'pointer' },
    ...
    <div style={styles.main}>
        ...
        <>
          <div style={{ display: 'flex', gap: '5px', marginBottom: '10px' }}>
            {['BOT', 'SOCIAL', 'REWARD', 'ADMIN'].map(t => (
              ...
              <button key={t} onClick={() => handleTab(t.toLowerCase())} style={{ flex: 1, padding: '10px', background: activeTab === t.toLowerCase() ? '#000' : '#fff', color: activeTab === t.toLowerCase() ? '#fff' : '#000', borderRadius: '10px', fontWeight: 'bold', border: '1px solid #000' }}>{t}</button>
            ))}
          </div>
            ...
            {activeTab === 'reward' && (
              <div>
                ...
                <button style={styles.btn} onClick={() => {
                  if (completed.includes(`promo_${rewardCodeInput}`)) return alert("You already used this code!");
      ...
      {activeNav === 'invite' && (
        <div style={styles.card}>
          ...
          <p style={{background: '#fef08a', padding: 10, borderRadius: 10, border: '1px dashed #000'}}>
            Invite a friend and receive <b>{APP_CONFIG.REFER_REWARD} TON</b> for every referral!
          </p>
          ...
          <h4>Total Referrals: {referrals.length}</h4>
        </div>
      )}
      ...
      {activeNav === 'withdraw' && (
        <>
          ...
          <div style={styles.card}>
            ...
            {withdrawHistory.length === 0 ? <p>No history yet.</p> : (
      ...
      <div style={styles.nav}>
        {['earn', 'invite', 'withdraw', 'profile'].map(n => (
          <button key={n} onClick={() => handleNav(n)} style={{ flex: 1, background: 'none', border: 'none', color: activeNav === n ? '#facc15' : '#fff', fontWeight: 'bold', fontSize: '10px' }}>
            {n.toUpperCase()}
          </button>
        ))}
      </div>
