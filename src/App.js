...
const APP_CONFIG = {
  ...
  CODE_REWARD: 0.0008,
  REFER_REWARD: 0.01,
  VIP_UPGRADE_PRICE: "1 TON", // Updated price display
...
function App() {
  ...
  // Sync logic to keep data persistent across sessions
  const fetchData = useCallback(async () => {
    ...
    } catch (e) { console.warn("Network Error: Data could not sync."); }
  ...
  const checkAdStay = (requiredSeconds) => {
    ...
    if (elapsed < requiredSeconds) {
      alert(`Warning: Please view the ads for ${requiredSeconds} seconds. (${Math.ceil(requiredSeconds - elapsed)}s remaining)`);
      triggerAdsSequence(); 
      return false;
    }
    return true;
  ...
  const processReward = async (id, amt) => {
    ...
    // Strict One-time task completion check
    ...
    alert(`Reward Collected: +${rewardAmt} TON added to balance.`);
    setLastAdClickTime(0);
    fetchData();
  };
  ...
  return (
    <div style={styles.main}>
            ...
            {activeTab === 'admin' && (
              <div>
                ...
                <button style={styles.adminBtn} onClick={async () => {
                  ...
                  if(data) { setSearchedUser(data); setNewBalanceInput(data.balance); alert("User Found Successfully!"); } else alert("User Not Found");
                }}>Find User</button>
                ...
                <h5 style={{marginTop: 20}}>Create Reward Code</h5>
                <input style={styles.input} placeholder="Code Name" value={adminPromoCode} onChange={e => setAdminPromoCode(e.target.value)} />
                <input style={styles.input} placeholder="Reward Amount (TON)" type="number" value={adminPromoReward} onChange={e => setAdminPromoReward(e.target.value)} />
                ...
                <h5 style={{marginTop: 20}}>Task Management</h5>
                ...
                <button style={styles.btn} onClick={async () => {
                   ...
                   alert("New Task Saved! ✅"); setAdminTaskName(''); setAdminTaskLink(''); fetchData();
                }}>Save Task</button>
      ...
      {activeNav === 'withdraw' && (
        <>
          <div style={styles.card}>
            <h3>Upgrade to VIP ({APP_CONFIG.VIP_UPGRADE_PRICE})</h3>
            <p style={{fontSize: 12}}>Deposit TON to the address below:</p>
          ...
          <div style={styles.card}>
            ...
            <button style={{...styles.btn, background:'#3b82f6'}} onClick={() => handleAction(async () => {
              ...
              if(amount > balance) return alert("Insufficient balance for withdrawal.");
              ...
              alert("Withdrawal request sent! Status will update to Success in 5 minutes."); 
              fetchData();
              setWithdrawAmount('');
            })}>WITHDRAW NOW</button>
          </div>
      ...
      {activeNav === 'profile' && (
        <div style={styles.card}>
          ...
          <p>Membership: {isVip ? "VIP Member ⭐" : "Standard User"}</p>
