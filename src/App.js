...
function App() {
  ...
  const styles = {
    ...
    copyBtn: { background: '#334155', color: '#fbbf24', border: 'none', padding: '5px 10px', borderRadius: '5px', fontSize: '10px', cursor: 'pointer' },
    ...
    <div style={styles.main}>
          ...
          {activeTab === 'bot' && botTasks.map(b => (
            <div key={b.id} style={styles.card}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                ...
                <button onClick={() => copyToClipboard(b.link)} style={styles.copyBtn}>COPY LINK</button>
              </div>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '11px', background: '#0f172a', padding: '12px', borderRadius: '10px', marginBottom: '10px', border: '1px solid #334155' }}>
                        <div style={{display:'flex', justifyContent:'space-between', marginBottom:'10px'}}>
                          <span>Address: <small style={{color:'#fbbf24'}}>{walletAddress.slice(0,12)}...</small></span>
                          <button onClick={() => copyToClipboard(walletAddress, "Address Copied!")} style={styles.copyBtn}>COPY</button>
                        </div>
                        <div style={{display:'flex', justifyContent:'space-between'}}>
                          <span>MEMO ID: <b style={{color:'#fbbf24'}}>{userUID}</b></span>
                          <button onClick={() => copyToClipboard(userUID, "Memo Copied!")} style={styles.copyBtn}>COPY</button>
                        </div>
                      </div>
      ...
      {activeNav === 'invite' && (
        <div style={{ textAlign: 'center' }}>
          <div style={{ ...styles.card, border: '1px solid #fbbf24' }}>
            ...
            <div style={{ background: '#0f172a', padding: '15px', borderRadius: '15px', marginBottom: '15px' }}>
              <p style={{ fontSize: '14px', margin: '0 0 5px 0' }}>
                Earn <b style={{ color: '#fbbf24' }}>0.0005 TON</b> per friend
              </p>
              ...
              <p style={{ fontSize: '11px', color: '#94a3b8', marginTop: '5px' }}>
                (Earn from every task they complete)
              </p>
            </div>
          ...
          <h3 style={{ marginTop: '20px', textAlign: 'left', paddingLeft: '5px' }}>INVITE HISTORY</h3>
          <div style={styles.card}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span>Total Invited:</span>
              <span style={{ color: '#fbbf24', fontWeight: 'bold' }}>{invites} Users</span>
            </div>
          </div>
        </div>
      )}
