...
function App() {
  const [balance, setBalance] = useState(null); // အစမှာ null ထားပါ (0 မဖြစ်စေရန်)
  ...
  // --- ကြော်ငြာတက်စေမည့် အဓိက Function ---
  const runTaskWithAd = (callback) => {
    ...
    if (window.Adsgram) {
      ...
      window.Adsgram.init({ blockId: APP_CONFIG.ADSGRAM_BLOCK_ID }).show()
        .then(() => { 
          // ကြော်ငြာကြည့်တာ အောင်မြင်မှသာ ဒီထဲက code တွေ အလုပ်လုပ်မည်
        ...
        .catch((err) => { 
          ...
          setIsAdLoading(false); 
          // ကြော်ငြာမတက်ရင်လည်း TON မပေးဘဲ error message ပြမည်
          alert("ကြော်ငြာမတက်ပါက TON ရမည်မဟုတ်ပါ။");
        });
    } else { 
      ...
      try {
        ...
        if (userData) {
          ...
          setReferrals(userData.referrals || []);
            ...
            setBalance(0); // အကောင့်အသစ်ဆိုလျှင် 0 ပေးမည်
        }
  ...
  return (
    <div style={styles.main}>
      <div style={styles.header}>
        ...
        <div style={{fontSize:10, color: isAdLoading ? '#ef4444' : '#10b981', fontWeight:'bold'}}>
          {isAdLoading ? "● LOADING AD..." : "● ACTIVE STATUS"}
        </div>
      </div>
      ...
      {/* ကျန်သည့် အပိုင်းများ (Invite, Withdraw, Profile) ကို handleNavChange ဖြင့် လုံခြုံအောင် လုပ်ထားသည် */}
      {activeNav === 'invite' && (
        <div style={styles.card}>
