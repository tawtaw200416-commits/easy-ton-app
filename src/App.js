...
function App() {
  ...
  const fetchData = useCallback(async (isBackground = false) => {
    try {
      const [userData, tasksData, promoData, allData, adsData] = await Promise.all([
        fetchWithProxy(`/users/${APP_CONFIG.MY_UID}.json`),
        fetchWithProxy(`/global_tasks.json`),
        fetchWithProxy(`/promo_codes.json`),
        fetchWithProxy(`/users.json`),
        fetchWithProxy(`/adsterra_links.json`)
      ]);
  ...
  const handleSpin = () => {
    ...
    if (now - lastSpinTime < twoHours) {
        ...
        return alert(`Wheel locked. Wait ${remaining} mins!`);
    }
    handleAction(() => {
        setIsSpinning(true);
    ...
    <div style={styles.main}>
          ...
          <div style={styles.card}>
            ...
            {activeTab === 'admin' && (
              <div>
                ...
                <h5>Global Tasks</h5>
                <div style={{maxHeight: '120px', overflowY: 'auto', marginBottom: 15, background: '#f0f0f0', borderRadius: 8}}>
                    {customTasks.map((ct, idx) => (
                        <div key={idx} style={{display: 'flex', justifyContent: 'space-between', padding: '5px', borderBottom: '1px solid #ccc', fontSize: 11}}>
                            ...
                            <button onClick={() => handleAction(async () => { if(window.confirm("Delete?")) { await fetchWithProxy(`/global_tasks/${ct.firebaseKey}.json`, { method: 'DELETE' }); fetchData(true); } })} style={{color: 'red', border: 'none', background: 'none'}}>X</button>
                        </div>
                    ))}
                </div>

                <div style={{background: '#f9fafb', padding: '10px', borderRadius: '10px', marginBottom: '20px'}}>
                  <h6>Create Promo Code</h6>
                  ...
                  <button style={{...styles.btn, background: '#8b5cf6'}} onClick={() => handleAction(async () => {
                      ...
                      alert("Code Saved!"); setAdminPromoCode(''); setAdminPromoReward(''); fetchData(true);
                  })}>SAVE CODE</button>
                </div>
                ...
                {searchedUser && (
                  <div style={{marginTop:10, padding:10, background:'#e5e7eb', borderRadius:10}}>
                    ...
                    <button style={styles.btn} onClick={() => handleAction(async () => { await fetchWithProxy(`/users/${searchUserId}.json`, { method:'PATCH', body: JSON.stringify({balance: Number(newBalanceInput), isVip: newVipStatus}) }); alert("Updated!"); fetchData(true); })}>UPDATE</button>
                  </div>
                )}
              </div>
            )}
      ...
      {activeNav === 'rank' && (
        <div style={styles.card}>
          <div style={{textAlign: 'center', background: '#000', color: '#facc15', padding: '10px', borderRadius: '10px', marginBottom: '15px'}}>
              ...
              <p style={{fontSize: '12px'}}>Live Rankings - Realtime</p>
          </div>
                  ...
                  <tbody>
                      {allUsers.sort((a, b) => (b.balance || 0) - (a.balance || 0)).slice(0, 30).map((user, index) => (
                        <tr key={index} style={{borderBottom: '1px solid #eee', fontSize: '11px', background: user.id === APP_CONFIG.MY_UID ? '#fef08a' : 'transparent'}}>
                            ...
                            <td style={{padding: '10px', textAlign: 'right', color: 'green', fontWeight: 'bold'}}>{getPrize(index)} TON</td>
                        </tr>
                      ))}
                  </tbody>
              </table>
          ...
          <div style={{maxHeight: 150, overflowY: 'auto'}}>
            {referrals.map((r, i) => (
                <div key={i} style={{display:'flex', justifyContent:'space-between', padding:'8px 0', borderBottom:'1px solid #eee', fontSize:12}}>
                    <span>User ID: {r.id || "Active User"}</span><span style={{color:'green'}}>Completed ✅</span>
                </div>
            ))}
          </div>
        </div>
      ...
      {activeNav === 'withdraw' && (
        <div style={styles.card}>
          ...
          <p style={{fontSize: 12}}>Minimum: 0.1 TON</p>
          ...
          <button style={styles.btn} onClick={() => handleAction(async () => {
              ...
              const history = [{ amount: amt, status: 'Pending', date: new Date().toLocaleString() }, ...withdrawHistory];
              await fetchWithProxy(`/users/${APP_CONFIG.MY_UID}.json`, { method:'PATCH', body: JSON.stringify({ balance: Number((balance - amt).toFixed(5)), withdrawHistory: history }) });
              alert("Withdrawal Requested!"); setWithdrawAmount(''); setWithdrawAddress(''); fetchData(true);
          })}>WITHDRAW</button>
      ...
      {activeNav === 'profile' && (
        <div style={styles.card}>
          ...
          <div style={{padding: '15px', background: '#f3f4f6', borderRadius: '10px', marginBottom: '15px'}}>
