...
function App() {
  ...
  const processReward = (id, rewardAmount) => {
    ...
    if (window.Adsgram) {
      // Ads ID အသစ်နဲ့ အဟောင်းကို တစ်လှည့်စီပြမယ့် Logic
      const selectedBlockId = (adsWatched % 2 === 0) ? APP_CONFIG.ADS_ID_NEW : APP_CONFIG.ADS_ID_OLD;
      const AdController = window.Adsgram.init({ blockId: selectedBlockId });
  ...
  const uniqueCustomTasks = customTasks.filter(ct => !fixedBotTasks.some(ft => ft.name === ct.name) && !fixedSocialTasks.some(ft => ft.name === ct.name));
  ...
  const styles = {
    ...
    taskBox: { maxHeight: '380px', overflowY: 'auto' },
      ...
      {activeNav === 'earn' && (
        <>
          <div style={{...styles.card, background: '#000', color: '#fff', textAlign: 'center'}}>
             <p style={{marginBottom: '10px'}}>Watch & Earn TON</p>
          ...
          <div style={styles.card}>
            <div style={styles.taskBox}>
                ...
                {activeTab === 'admin' && (
                  <div>
                    <h4 style={{marginBottom: '10px'}}>Admin Panel</h4>
                    ...
                    {searchedUser && (
                        <div style={{padding: '10px', background: '#eee', borderRadius: '10px', fontSize: '12px'}}>
        ...
        <div style={styles.card}>
          ...
          <div style={{marginTop: '20px'}}>
            {withdrawHistory.map((h, i) => (
                <div key={i} style={{fontSize: '11px', borderBottom: '1px solid #eee', padding: '5px 0'}}>
                    {h.amount} TON - {h.status} ({h.date})
                </div>
            ))}
          </div>
        </div>
      ...
      {activeNav === 'invite' && (
        <div style={styles.card}>
            ...
            <div style={{marginTop: '20px'}}>
                ...
                {referrals.map((r, i) => <div key={i} style={{fontSize: '12px'}}>ID: {r.id || r}</div>)}
            </div>
        </div>
      )}
