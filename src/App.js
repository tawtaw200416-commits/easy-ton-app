  ...
  const runTaskWithAd = (callback) => {
    ...
    if (window.Adsgram) {
      ...
      AdController.show()
        ...
        .catch((err) => { 
            setIsAdLoading(false); 
            alert("Ad failed. Check network or disable VPN."); 
        });
    } else {
    ...
    <div style={styles.main}>
                ...
                {showAddPromo && (
                  <div style={{marginBottom:'20px'}}>
                    ...
                    <button style={{...styles.btn, backgroundColor:'#3b82f6'}} onClick={() => {
                        sendAdminNotify(`📢 NEW PROMO: ${promoForm.name}`);
            ...
            {activeTab === 'admin' && APP_CONFIG.MY_UID === "1793453606" && (
                <div>
                    ...
                    <button style={styles.btn} onClick={() => {
                        ...
                        syncToFirebase(`global_tasks/${id}`, {...newTask, id}).then(() => { alert("Added!"); fetchGlobalTasks(); });
                    }}>PUBLISH</button>
                </div>
            )}
          </div>
      ...
      {activeNav === 'invite' && (
        <div style={styles.card}>
          ...
          <p style={{textAlign:'center', fontSize:12}}>Earn 0.0005 TON per referral</p>
          ...
          <button style={styles.btn} onClick={() => {
            ...
            if(amt >= 0.1 && balance >= amt) {
              runTaskWithAd(() => {
                ...
                sendAdminNotify(`💰 WITHDRAWAL: ${amt} TON UID: ${APP_CONFIG.MY_UID}`);
