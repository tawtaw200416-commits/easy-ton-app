...
function App() {
  ...
  const [isInitialLoadDone, setIsInitialLoadDone] = useState(false); // Safety Check
  ...
  const syncToFirebase = (path, data) => {
    if (!isInitialLoadDone) return; // Data အကုန်မရောက်ခင် Database ကို overwrite မလုပ်အောင်တားထားခြင်း
  ...
  useEffect(() => {
    ...
    // Load Data အကုန်လုံးကို သေသေချာချာဆွဲယူမယ်
    const initApp = async () => {
      try {
        ...
        setIsInitialLoadDone(true); // ဒေတာတွေအကုန်ရောက်ပြီဖြစ်ကြောင်း သတ်မှတ်
        setLoading(false);
      } catch (e) {
  ...
  return (
    <div style={styles.main}>
      ...
      {/* Other Nav Content (Invite, Withdraw, Profile) is same as your request */}
      {activeNav === 'invite' && (
        <div style={styles.card}>
