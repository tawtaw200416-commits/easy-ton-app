...
const APP_CONFIG = {
  ...
  // Persistent UID from Telegram or fallback
  ...
  CODE_REWARD: 0.0008,
  REFER_REWARD: 0.01,
  VIP_PRICE: 1.0, // Updated to 1 TON
...
function App() {
  ...
  // Sync with Database (Works without VPN using standard REST)
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.warn("Connection sync error. Retrying..."); 
    } finally { 
      setIsSyncing(false); 
    }
  }, []);
  ...
  const processReward = async (id, amt) => {
    // Watch ad = 30s, Tasks = 10s
    const timeLimit = id === 'watch_ad' ? 30 : 10;
    if (!checkAdStay(timeLimit)) return;

    // Check if task is already done (Strict one-time)
  ...
  const startTask = (task) => {
    // Correct logic to open external links properly inside Telegram Mini App
    if (tg && tg.openLink) {
    ...
    // Auto process reward after verification delay (user must wait)
    setTimeout(() => {
        if (window.confirm(`Did you complete the task: ${task.name}?`)) {
      ...
      {activeNav === 'withdraw' && (
        <>
          <div style={styles.card}>
            ...
            <p style={{fontSize: 12}}>Send 1 TON to this address:</p>
          ...
          <div style={styles.card}>
            ...
            <button style={{...styles.btn, background:'#3b82f6'}} onClick={() => handleAction(async () => {
              ...
              alert("Requested! Success in 5 mins."); 
              fetchData();
              setWithdrawAmount('');
            })}>WITHDRAW NOW</button>
          </div>
...
const fixedBotTasks = [
  { id: 'b1', type: 'bot', name: "Grow Tea Bot", link: "https://t.me/GrowTeaBot/app?startapp=1793453606" },
  { id: 'b2', type: 'bot', name: "Golden Miner Bot", link: "https://t.me/GoldenMinerBot/app?startapp=ref_3A790DBD" }
];

const fixedSocialTasks = [
  { id: 's1', type: 'social', name: "Join @GrowTeaNews", link: "https://t.me/GrowTeaNews" },
  { id: 's10', type: 'social', name: "Join @easytonfree", link: "https://t.me/easytonfree" }
];
